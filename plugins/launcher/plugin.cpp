...
MAKE_SINGLETON_FACTORY(LauncherPanelPlugin)

void BackendPlugin::registerTypes(const char *uri)
{
    Q_ASSERT(uri == QLatin1String("Ubuntu.SystemSettings.Launcher"));
    qmlRegisterSingletonType<LauncherPanelPlugin>(
        uri, 1, 0, "LauncherPanelPlugin", LauncherPanelPlugin_singleton_factory
    );
}
